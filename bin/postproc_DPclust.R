...
for(i in 1:length(DPclustsres.files)){
  ...
  dataset_fracs = bind_cols(chr=dataset$chromosome[,1] , pos=dataset$position[,1], subclonal.fractions=dataset$subclonal.fraction , best.assignment.likelihoods=clustering$best.assignment.likelihoods, 
                            all.assignment.likelihoods=clustering$all.assignment.likelihoods  , Cluster=clustering$best.node.assignments, DP=dataset$WTCount+dataset$mutCount, 
  ...
  if( length(clust_prop[clust_prop>=0.025])==1 ){#if single cluster
    dataset_fracs$all.assignment.likelihoods = 1
    dataset_fracs$Cluster = names(clust_prop[clust_prop>=0.025])
    dataset_fracs$best.assignment.likelihoods = names(clust_prop[clust_prop>=0.025])
      }else{#if single clusters
  ...
  clustering$cluster.locations =  clustering$cluster.locations[clustering$cluster.locations[,1] %in% unique(dataset_fracs$Cluster),]
  ...
  clustering$cluster.locations[,1][clustering$cluster.locations[,1]%in%clonalclusts] = 1
  ...
  cluster.locations = as.data.frame(clustering$cluster.locations)
  colnames(cluster.locations) = c("cluster","proportion","n_ssms")
  cluster.locations$clonal = (clustering$cluster.locations[,2] >=0.95)
  cluster.locations$cluster = as.numeric(factor(cluster.locations$cluster))-2
  
  write_tsv(cluster.locations %>% filter(!clonal),file = "cluster_locations.tsv")
  ...
  if(length(clonalclusts)==0){#when no clonal cluster detected, all alterations are considered subclonal
    ...
    if(length(clonalclusts)>1){#when there are multiple clonal clusters, sum their likelihoods
      ...
      all.assignment.likelihoods.tmp[,as.character(clonalclusts[1])] = rowSums(all.assignment.likelihoods.tmp[,as.character(clonalclusts)])
      cols_to_keep = colnames(all.assignment.likelihoods.tmp)
      cols_to_keep = cols_to_keep[!cols_to_keep%in%as.character(clonalclusts[-1])]
      dataset_fracs$all.assignment.likelihoods = all.assignment.likelihoods.tmp[,cols_to_keep]
      
      if(is.null(ncol(dataset_fracs$all.assignment.likelihoods))){ #if single cluster after merge
        dataset_fracs$Clonal[dataset_fracs$all.assignment.likelihoods>=0.95 ] = TRUE
      }else{
      ...
      if(length(clust_prop[clust_prop>=0.025])>1){# if several clusters
        dataset_fracs$Clonal[which(dataset_fracs$all.assignment.likelihoods[,as.character(clonalclusts)]>=0.95) ] = TRUE
      if(ncol(dataset_fracs$all.assignment.likelihoods)>2){# if there more than 1 subclonal cluster, sum their likelihoods
