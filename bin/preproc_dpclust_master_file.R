...
if (!is.null(opt$rho_and_psi)) {
  rho_and_psi_files = unlist(strsplit(opt$rho_and_psi, ","))
} else {
...
createProjectFile(outputfile=outputfile, donornames=donornames, samplenames=samplenames, 
                  sex=sex, purities=purities, rho_and_psi_files=rho_and_psi_files)
